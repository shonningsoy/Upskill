...
- **Iceberg, external catalog (e.g. AWS Glue / Iceberg REST):** Snowflake reads tables that another engine owns and writes.
...
- **Iceberg with an external catalog is read-only in Snowflake** — the catalog owner is the only safe writer.
...
| External catalog | Glue / Iceberg REST owns metadata; Snowflake reads | Fits when Spark/Trino already own the tables (read-only in SF) |
...
4. **Iceberg table:** choose a **catalog** — Snowflake-managed (writable, fast) or external (read-only in Snowflake).
...
flowchart TD
    ...
    CAT -->|External e.g. Glue| R[Read-only in Snowflake<br/>other engine writes]
...
-- Iceberg reading an external catalog (Glue): read-only in Snowflake
...
- **Risk or governance angle:** Governance is strongest when Snowflake owns the metadata (internal or Snowflake-managed Iceberg). With an external catalog, Snowflake is a read-only guest and masking/RBAC coverage is more limited.
...
- **Choosing an external catalog then expecting to write from Snowflake** — the catalog owner is the writer; Snowflake is read-only there.
...
| Another engine (Spark/Trino) already owns the tables | **Iceberg (external catalog)** | Snowflake reads shared tables | Read-only in SF, weaker governance |
...
- How much governance (masking/RBAC) is realistically lost when Snowflake reads an external catalog?
